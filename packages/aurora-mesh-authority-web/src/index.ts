...
const browserWasmSource: MeshAuthorityWasmSource = {
  ...
  // not present at typecheck time, and a browser wants the absolute URL anyway
  // so the generated loader can find its `.wasm` beside the JS.
  importBindings: async () =>
    await import(/* @vite-ignore */ new URL('./wasm/aurora_mesh_authority.js', import.meta.url).href)
}
