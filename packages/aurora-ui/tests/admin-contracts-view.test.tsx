...
describe('admin contracts view', () => {
  it('renders /admin/contracts browser status from live SDK registry and capability catalog data', async () => {
    ...
    expect(snapshot.contracts[0]?.exportState).toContain('Aurora lists this action as ready.')

    expect(markup).toContain('Review which Aurora actions are available')
    expect(markup).toContain('Search actions')
    expect(markup).toContain('Service')
    expect(markup).toContain('Action detail')
    expect(markup).toContain('Weather service / 1 action(s)')
    expect(markup).toContain('Weather Forecast')
    expect(markup).toContain('Available from this screen')
    expect(markup).toContain('Payload details are available for this action')
