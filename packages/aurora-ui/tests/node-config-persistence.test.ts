...
class DelayedMapVaultStorage extends MapVaultStorage {
  private nodeConfigWrites = 0

  override async set(key: string, value: unknown): Promise<void> {
    if (key === BROWSER_PEER_NODE_CONFIG_DOCUMENT_KEY) {
      this.nodeConfigWrites += 1
      if (this.nodeConfigWrites === 1) {
...
describe('browser node config persistence', () => {
  ...
  it('serializes concurrent policy saves so the newest document remains durable', async () => {
    const storage = new DelayedMapVaultStorage()
    const store = new BrowserPersistentPeerCredentialStore({
      storage,
      crypto: globalThis.crypto,
    ...
    const first = emptyAuroraNodeConfigDocument(710)
    const second = emptyAuroraNodeConfigDocument(720)
    second.services.tts = { routing: { prefer: 'network', fallback: 'local' } }

    await Promise.all([
      store.saveNodeConfigDocument(first),
      store.saveNodeConfigDocument(second),
    ])

    expect(await store.loadNodeConfigDocument()).toEqual(second)
  })
