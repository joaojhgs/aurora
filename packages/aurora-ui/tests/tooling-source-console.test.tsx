...
describe('source-first Tooling console', () => {
  ...
  it('renders a dark source-first operator console with truthful platform and management states', () => {
    const markup = renderToolsPanel('android')

    expect(markup).toContain('Global policy console')
    ...
    expect(markup).toContain('Future-tool inheritance')
    expect(markup).toContain('Approvals 2')
    ...
    expect(markup).toContain('Connect another MCP server')
    expect(markup).toContain('Android')
    expect(markup).toContain('Durable policy changes route through Tooling.SetPolicyMode')
    expect(markup).toContain('ALLOW NON-BLOCKED TOOLS · DENY ALL TOOLS · DRY RUN ONLY')
  })
})
