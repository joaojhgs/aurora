...
function deploymentModes(transportKind: string, snapshot: AuroraShellSnapshot): DeploymentModeCard[] {
  ...
  return [
    ...
    mode('ios-thin', 'iOS Thin', 'iOS client against a remote Gateway.', transportKind === 'native-mobile' && snapshot.nativePlatform.toLowerCase().includes('ios') ? 'available-remote' : 'unsupported', snapshot.nativePlatform, 'iOS remains app-owned surfaces only; Siri replacement is unsupported.'),
