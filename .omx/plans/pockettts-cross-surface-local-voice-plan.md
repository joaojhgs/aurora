...
**Stop condition:** implementation is complete only when every surface advertises honest capability, exactly one runtime owns capture and model inference for a session, native background turns no longer depend on a live WebView, pure web stops cleanly when browser lifecycle makes capture unreliable, model/runtime portability is proven per signed model pack, and release claims are backed by the required emulator, browser, desktop, and physical-device evidence.
