...
- Latest local feature-branch code checkpoint before this tracker refresh: `b3b76826`, with prior durable state/evidence checkpoints `052f49d6`, `557e740e`, and `f167eb86`. It includes PocketTTS bounded-stop test hardening, Tauri source-boundary scan stabilization, desktop live X-display/re-entry repairs, removal of superseded Waydroid evidence, the local coturn loopback-relay fixture repair, and comprehensive product-safe device labels across shell, diagnostics, invite, and hosted-connection fallbacks. The containing tracker commit adds the bounded API35 software-QEMU record. These commits are local only and have not been pushed.
...
This section is the authoritative resume state from the 2026-08-08 strict code-and-evidence audit, reconciled through integration checkpoint `f8cc01d8` plus the later Phase 7/8 adjudications and the 2026-08-11 final-source validation recorded below. Later sections retain chronological evidence and provenance only; they do not override this verdict. Older records that say MobileMCP was unavailable describe earlier attempts only; the authoritative final-source device record is `reports/native-voice/android-waydroid-20260811-final-8fff7af6/`.
...
- Resume order is mandatory from the local `feat/ui-multi-platform-integration` tip after `b3b76826` plus the containing tracker reconciliation commit: (1) retain RAC-25 as partial while production signing material, final artifact hashes, SBOM/license tooling, and external release evidence remain open; (2) finish RAC-27's durable integrated direct/STUN/TURN aggregate, including packaged desktop STUN/TURN, when Docker access is available; (3) rebuild the same dynamic Android APK from current source and resume Waydroid validation by completing make-device-available continuation, clean restart persistence, pairing, and a complete Gateway/UI voice turn after Waydroid can be restarted; (4) run CI-equivalent API35 validation when KVM is available; and (5) continue Phases 8-13 in dependency order without promoting unavailable physical/platform claims.
...
- Current code evidence baseline after the 2026-08-11 corrective update: `b3b76826` on `feat/ui-multi-platform-integration`; the branch includes accepted iOS start hardening, Android WebRTC harness hardening, voice-clone tombstone scrubbing, RAC-25 release-trust hardening plus the static release trust workflow gate and timeout-hardened focused guard tests, RAC-54 HTTP rollback role preservation, RAC-51 fail-closed credential-storage hardening, Android client bundle initialization, dynamic role preservation across disabled rollout gates, mesh-role/runtime-tier decoupling, Waydroid/MobileMCP evidence, UI copy gate repairs including product-safe device fallbacks and stable-peer-ID suppression, PocketTTS bounded-stop harness tightening, Tauri boundary-scan stabilization, desktop live X-display/re-entry repairs, and the local coturn loopback-relay fixture repair.
...
- RAC-27 and Android final validation remain pending. RAC-27 now has exact-source browser/Python direct and STUN passes plus a packaged Linux desktop direct pass; the forced-TURN root cause was repaired and a bounded diagnostic rerun passed, but its successful raw report was not persisted and the integrated packaged all-route aggregate remains blocked by Docker socket access. The redacted checkpoint is `reports/native-voice/rac27-live-20260811/`. Waydroid/MobileMCP evidence remains under `reports/native-voice/android-waydroid-20260811-final-8fff7af6/`: source commit `8fff7af6`, APK SHA-256 `83256515b81e87f3fd28e9a5122c0d6a5dd20d7b7507591daff475bc6de0f24c`, installed `base.apk` hash match, role picker with both dynamic role choices, Connect-to-Aurora saved profile state, invite/manual-address onboarding, and honest Offline connected-devices navigation when no Gateway is listening. Visual inspection of screenshots `01` through `06` confirms those claims and the black restart blocker; source/copy review found the exact implementation-facing fallback wording. Initial correction `f167eb86` and comprehensive follow-up `b3b76826` replace node/peer fallbacks with device-safe wording and suppress raw stable peer IDs. Fresh verification at `b3b76826` passes all `@aurora/ui` tests (`67` files, `700/700`), UI/Tauri/web typechecks, production-copy lint, and independent review with zero findings. Because the installed APK predates these source fixes, a future current-source Android rebuild/runtime check is still required. Direct ADB reconnect to `192.168.240.112:5555` returns `No route to host`. Make-this-device-available continuation, clean restart persistence, pairing, complete Gateway/UI voice turn, and physical-device proof remain unpromoted.
...
- RAC reconciliation: the authoritative evidence remains **22 pass / 24 partial / 6 withheld / 4 blocked** after RAC24 lifecycle closure, RAC26 persisted-fallback closure, RAC30 scope correction, and dynamic-role repair. MobileMCP evidence proves the tested APK renders both dynamic roles and completes the Connect-to-Aurora invite/manual-address path to an honest Offline state; the Waydroid container later stopped during WebView restart recovery and requires host authorization to restart. The API35 software-QEMU alternative reached ADB `device` at 164 seconds and package-manager readiness at 486 seconds, but `sys.boot_completed` stayed unset through 711 seconds, so maintained smoke was not run and the emulator was stopped cleanly. CI-equivalent accelerated validation remains blocked because `developer` cannot access `/dev/kvm`. Browser/Python direct and STUN pass, packaged desktop direct passes, and the repaired coturn fixture supports a successful bounded TURN diagnostic, but a durable integrated all-route aggregate is blocked because `developer` cannot access the Docker socket. Pairing, the complete Gateway/UI voice turn, installed-desktop background cutover, physical devices, and release gates remain open. Temporary dummy/direct-only wrapper attempts remain excluded from evidence.
...
- Superseded headless Android handoff: older notes said KVM-enabled `@aurora_api35_google` was healthy on `emulator-5560`, but current validation supersedes that. The current `developer` user lacks `/dev/kvm` access. A bounded `-accel off` attempt reached ADB and package-manager readiness but not strict boot completion, so no install/smoke claim was promoted; it was then cleaned up and the ADB inventory returned empty. Direct Waydroid reconnect to `192.168.240.112:5555` returns `No route to host`. Direct/STUN/TURN aggregate reruns still require Docker socket access. Local non-repo AVD data partition is 6G.
...
- Current local feature-branch code baseline is `b3b76826` on `feat/ui-multi-platform-integration`; no push was performed. The inventory estimate remains approximately **60-65% implemented** and **40-45% fully verified**. Six of fourteen phases (0-5) are complete for their scoped claims; Phase 6 is decision-complete only as withheld, and Phases 7-13 remain incomplete, partial, withheld, or blocked.
- Authoritative RAC counts remain unchanged at **22 pass, 24 partial, 6 withheld, and 4 blocked**. The canonical matrix is `reports/native-voice/native-voice-rac-matrix.json`; it uses `codeEvidenceBaselineHead` `b3b76826` and explicitly keeps the integrated all-route desktop aggregate, Android Gateway/UI completion, API35, physical mobile, signing/SBOM/license, and rollback evidence open.
...
- Android tested artifact: universal debug APK built from `8fff7af6`, SHA-256 `83256515b81e87f3fd28e9a5122c0d6a5dd20d7b7507591daff475bc6de0f24c`, size `341843160` bytes. Deltas through `84a6353d` did not affect packaged app code; corrections through `b3b76826` do affect presentation code, so this APK no longer represents the exact current source and must be rebuilt before the final Waydroid acceptance pass.
...
- API35/QEMU evidence: a bounded software-TCG attempt launched `aurora_api35_google` on `emulator-5560` with API35/x86_64, `-accel off`, no window/audio/boot animation, SwiftShader, and snapshots disabled. ADB reached `device` at 164 seconds and package-manager readiness at 486 seconds, but `sys.boot_completed` remained unset through the last 711-second poll. The maintained `android:smoke` runner was therefore not started. Cleanup succeeded and the post-run ADB inventory was empty. CI-equivalent accelerated validation remains blocked because the `developer` user lacks `/dev/kvm`; no install, rendered UI, lifecycle, voice, performance, or physical-device claim is promoted. Durable evidence is `reports/native-voice/api35-software-20260811/`.
