...
- Resume order is mandatory from the local `feat/ui-multi-platform-integration` tip after `63b25066` plus the containing current-source evidence commit: (1) retain RAC-25 as partial while production signing material, final artifact hashes, SBOM/license tooling, and external release evidence remain open; (2) finish RAC-27's durable integrated direct/STUN/TURN aggregate, including packaged desktop STUN/TURN, when Docker access is available; (3) install the already-built dynamic current-source Android APK on Waydroid and complete make-device-available continuation, clean restart persistence, pairing, and a complete Gateway/UI voice turn after Waydroid can be restarted; (4) rerun maintained API35 smoke on a KVM-capable or otherwise stable CI-equivalent emulator; and (5) continue Phases 8-13 in dependency order without promoting unavailable physical/platform claims.
...
- RAC-27 and Android final validation remain pending. RAC-27 has exact-source browser/Python direct and STUN passes, a packaged Linux desktop direct pass, and fresh current-source desktop profile/service/sidecar validation; the forced-TURN root cause was repaired and a bounded diagnostic rerun passed, but its successful raw report was not persisted and the integrated packaged all-route aggregate remains blocked by Docker socket access with no rootless/local-service alternative. The redacted checkpoint is `reports/native-voice/rac27-live-20260811/`. Waydroid/MobileMCP evidence remains under `reports/native-voice/android-waydroid-20260811-final-8fff7af6/` and is bound to source commit `8fff7af6` plus APK SHA-256 `83256515b81e87f3fd28e9a5122c0d6a5dd20d7b7507591daff475bc6de0f24c`; it proves the recorded dynamic-role onboarding and honest Offline navigation only. Current-source APK `c99008ea74dc6cd21996e0a0467a8c4d86ffafe819bf51e545514606700a946e` from `63b25066` now passes build, artifact policy, 14 client-bundle tests, 53 focused Android tests, and 27 preflight checks. The maintained API35 software-TCG smoke pushed the exact APK, then Android killed `system_server` during `pm install` before Aurora launched; no current-source UI/runtime claim follows. Direct ADB reconnect to `192.168.240.112:5555` still returns `No route to host`. Make-this-device-available continuation, clean restart persistence, pairing, complete Gateway/UI voice turn, and physical-device proof remain unpromoted.
