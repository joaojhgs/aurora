...
- Resume order is mandatory from the local `feat/ui-multi-platform-integration` tip after `84a6353d`: (1) retain RAC-25 as partial while production signing material, final artifact hashes, SBOM/license tooling, and external release evidence remain open; (2) finish RAC-27's durable integrated direct/STUN/TURN aggregate, including packaged desktop STUN/TURN, when Docker access is available; (3) resume Android final-source validation with the same dynamic APK by completing make-device-available continuation, clean restart persistence, pairing, and a complete Gateway/UI voice turn after Waydroid can be restarted; and (4) continue Phases 8-13 in dependency order without promoting unavailable physical/platform claims.
