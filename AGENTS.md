...
This project is indexed by GitNexus as **aurora** (24387 symbols, 86284 relationships, 300 execution flows).
...
- Local UI polish uses **one** Next debug server: `pnpm dev:ui:debug` (same as `pnpm dev:web` in this worktree). Switch surface/role/admin at runtime with query params, the in-app Development preview picker, or the `aurora-debug-ui` cookie/sessionStorage. Do not spawn 11 Next processes for chrome comparison. Named `pnpm dev:ui:<preset>` commands remain valid isolated wrappers (random port) that apply the same query string; they must not bake a unique `NEXT_PUBLIC_*` role/surface into the compile. Production clients ignore the override. Query contract for agents: `aurora-surface=web|desktop-local|desktop-thin|android|ios`, `aurora-role=remote-console|mesh-node|python-full`, `aurora-admin=0|1`. Example: `http://127.0.0.1:3000/?aurora-surface=android&aurora-role=mesh-node&aurora-admin=0`. Active override JSON: `GET /__aurora/debug-preset`.
