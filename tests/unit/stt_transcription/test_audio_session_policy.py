"""Tests for Transcription.ProcessAudio audio session policy."""
...
@pytest.mark.asyncio
async def test_transcription_streaming_denies_missing_selector(service, mock_bus):
...
@pytest.mark.asyncio
async def test_transcription_streaming_denies_missing_consent_token(service, mock_bus):
...
@pytest.mark.asyncio
async def test_transcription_streaming_validates_session_and_accepts_chunk(service, mock_bus):
    ...
    assert mock_bus.publish.call_args.args[1].event_type == "transcription_audio_accepted"


@pytest.mark.asyncio
async def test_transcription_streaming_rejects_invalid_sample_format(service, mock_bus):
...
@pytest.mark.asyncio
async def test_transcription_streaming_denies_failed_session_validation(service, mock_bus):
