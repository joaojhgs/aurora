...
class TestBasicConversationFlow:
    ...
    async def test_basic_conversation(self, mock_environment):
        ...
        with patch('app.database.database_manager.DatabaseManager', return_value=mock_environment["database"]):
            with patch('app.config.config_manager.ConfigManager.get_instance', return_value=mock_environment["config"]):
