...
test('harness source preserves real-peer, Python-free desktop-client, driver, and scanner gates', async () => {
  ...
  assert.match(source, /AURORA_DESKTOP_LIVE_E2E_SCRIPT_TIMEOUT_MS/)
  ...
  assert.match(source, /driverRun\.exit\.code/)
  assert.match(source, /await waitForChild\(pythonPeer, 30_000, 'Python peer'\)/)
...
test('webdriver fixture source invokes the narrow desktop live WebView hook', async () => {
  ...
  assert.match(source, /selectMainDesktopWindow/)
  assert.match(source, /window\/handles/)
  assert.match(source, /overlaySurface/)
  assert.match(source, /waitForDesktopHookContext/)
  assert.match(source, /desktop-webview-hook-not-ready/)
  assert.match(source, /isTransientDesktopContextError/)
