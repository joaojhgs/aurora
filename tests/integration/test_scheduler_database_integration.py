...
from app.database import CronJob, ScheduleType, JobStatus, SchedulerDatabaseService
...
class TestSchedulerDatabaseIntegration:
    ...
    async def scheduler_manager(self, db_service):
        ...
        with patch('app.database.scheduler_service.SchedulerDatabaseService', return_value=db_service):
    ...
    async def test_failed_job_logging(self, db_service, scheduler_manager):
        ...
        test_job = Job(
            ...
            created_at=now,
            updated_at=now,
            metadata={"description": "Job that will fail", "command": "invalid_command_that_will_fail"}
        )
