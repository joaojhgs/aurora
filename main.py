...
def main_with_ui():
    ...
    bridge_future = asyncio.run_coroutine_threadsafe(ui_bridge.start(), supervisor_loop)
    ...
    if supervisor_loop and not supervisor_loop.is_closed():
        ...
        try:
            asyncio.run_coroutine_threadsafe(signal_shutdown(), supervisor_loop).result(timeout=2.0)
        except Exception as e:
