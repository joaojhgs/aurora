#!/usr/bin/env python3
"""Generate Aurora's metadata-only sherpa-onnx Piper voice catalog."""
...
CHECKSUM_URL = (
    f"https://github.com/{REPOSITORY}/releases/download/tts-models/checksum.txt"
)
...
def _parse_voice_name(filename: str) -> tuple[str, str, str | None, str | None]:
    ...
    quality = parts[quality_index].lower() if parts[quality_index].lower() in _QUALITY_TOKENS else None
