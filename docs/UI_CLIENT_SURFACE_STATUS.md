...
- Desktop client source, bundle proof, native assistant-provider composition, and Linux native WebRTC fallback are implemented, but the packaged Python-free desktop live E2E still requires a fresh passing `pnpm test:desktop-client:live` report. **FINAL_STATUS_UPDATE_REQUIRED:** replace this sentence and the desktop row below after that live rerun passes.
...
| Android client debug APK | `pnpm --filter @aurora/tauri-ui android:build:client:apk` then `android:verify:client:apk` | `apps/aurora-tauri/reports/android-client-apk-{build-provenance,artifact-proof}.json` | Artifact lane exists and remains the required APK content proof; run it fresh when finalizing release evidence. It checks runtime-profile endpoint selection, invite-first setup, no forbidden Python/sidecar matches, and least-privilege client/mobile capabilities. |
| Android client debug AAB | `pnpm --filter @aurora/tauri-ui android:build:client:aab` then `android:verify:client:aab` | `apps/aurora-tauri/reports/android-client-aab-{build-provenance,artifact-proof}.json` | Artifact lane exists and remains the required AAB content proof; run it fresh when finalizing release evidence. It checks endpoint-agnostic policy and Python-free client/mobile mesh capabilities. |
...
The following checks summarize the current local integration evidence. Older rows remain useful only inside the named boundary; do not treat them as a replacement for the current completion gate.
...
| Desktop client package/live | Source and package gates are implemented. A fresh packaged Python-free live desktop E2E is pending; update the desktop status after `pnpm test:desktop-client:live` passes. |
...
- Reconfigure the saved thin profile when the operator Gateway or signaling endpoint changes; rebuilt artifacts are not required for ordinary endpoint changes.
