...
| Android thin packages | Endpoint-agnostic debug APK/AAB wrappers and artifact proof accept runtime HTTP/HTTPS/WS/WSS profiles, reject compiled operator endpoints, and scan for zero forbidden Python/sidecar matches. The current universal APK result and hash are refreshed whenever the runtime-configurable phone artifact is rebuilt. |
