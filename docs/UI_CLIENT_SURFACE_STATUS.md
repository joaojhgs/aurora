...
| iOS MobileSafari/WKWebView ↔ Python WebRTC | One normal serial E2E test, macOS workflow wiring, shared assertion/scanner reuse, separate per-surface reports, packaged-app path scanning, and Linux skip/transform/typecheck gates are implemented. Passing MobileSafari and packaged-Tauri-WKWebView simulator reports are pending the first unpushed macOS CI run; physical-device direct/STUN/TURN evidence remains separate. |
