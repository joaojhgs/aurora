...
- Android foreground WebView microphone policy, lifecycle release policy, and Keystore-backed peer credential/proof commands are implemented and build-proven. Python, TypeScript, Rust, Kotlin, and Swift use the same canonical `mesh_auth_proof_v1` HMAC transcript, including Python-compatible ASCII escaping for Unicode peer/room values. The packaged thin APK passes API 30 System WebView UI/native-payload E2E locally, including responsive bottom navigation and redacted native metadata. A physical Android 16 POCO X7 Pro also opened and cancelled the native QR scanner without stale pending UI/error state, requested the Android microphone grant from the exact `tauri.localhost` app origin, and completed a foreground `getUserMedia({audio:true})` capture/stop cycle. Do not claim durable Android background wakeword.
...
- iOS thin source wiring is implemented: the Tauri runtime selects the shared WebView HTTP/WebRTC path, the Rust commands route to Swift, peer reconnect bearer material and room secrets are held in device-only/non-synchronizing Keychain items, and nonsecret profiles use `UserDefaults`. The supported simulator build wrapper generates a temporary Python-free overlay with general runtime HTTP/HTTPS/WS/WSS connect policy and records successful-build provenance. This environment cannot produce Xcode artifacts, so iOS remains build/live-limited until macOS simulator/device direct-STUN-TURN, microphone/lifecycle, signing, and App Store/TestFlight evidence passes.
...
| --- | --- | --- | --- |
| Hosted web thin over HTTP | **Implemented, bounded** | `apps/aurora-web/app/aurora-client.ts` selects HTTP transport from the saved runtime profile and gates first launch through onboarding when no profile exists. | Deployment-specific TLS/CORS/auth and operator endpoint rollout remain environment-specific. |
...
| Desktop Tauri thin over HTTP/WebRTC | **Implemented, build-proven** | Async nonsecret profile load/edit/save/select reconstructs the shared WebView runtime for `http-only`, `webrtc-only`, or `webrtc-preferred`. First-run onboarding accepts a node name plus invite and derives signaling/pairing material; advanced endpoint/profile controls remain in normal settings. Desktop keychain stores room secrets separately from the profile. Fresh desktop-thin proof uses general connect policy and passes with no forbidden Python/sidecar matches. | Live desktop WebView-specific WebRTC smoke beyond the shared browser-engine harness remains environment-specific. |
| Android thin over HTTP/WebRTC | **Implemented, build- and UI/runtime-proven; WebRTC CI gates pending** | Android thin overlay, native plugin sync, Keystore peer credential/proof and room-secret commands, WebView microphone policy, lifecycle status, arm64 debug APK proof, x86_64 emulator proof, and universal debug AAB proof exist. Fresh APK/AAB builds use runtime-configurable endpoint profiles and general connect policy; artifact proof passed. The packaged APK passes API 30 UI/native-payload E2E. On a physical Android 16 device, camera permission, QR scanner open/cancel, cancellation recovery, microphone permission mediation, and foreground WebView audio capture passed. API 35 CI now runs both packaged-System-WebView and standalone-Android-Chrome forced-TURN pairing/RPC/reconnect/revocation against an external Python peer; local diagnostics retain explicit direct/STUN/TURN selection. | Fresh passing API 35 WebRTC evidence remains pending. Release signing, Play/App Distribution, additional OEM microphone/lifecycle behavior, physical STUN/TURN WebRTC paths, and durable background voice remain unproven. |
...
| --- | --- | --- | --- |
| Desktop thin AppImage/deb | `pnpm --filter @aurora/tauri-ui build:bundle:desktop-thin` | `apps/aurora-tauri/reports/desktop-thin-bundle-{prepare,proof}.json` | Passed; endpoint selection is runtime profile/onboarding state, the generated policy allows general HTTP/HTTPS/WS/WSS connect targets, and artifact proof checks no forbidden Python/sidecar matches, no external binaries/resources, and `aurora-thin` capability. |
| Android thin debug APK | `pnpm --filter @aurora/tauri-ui android:build:thin:apk` then `android:verify:thin:apk` | `apps/aurora-tauri/reports/android-thin-apk-{build-provenance,artifact-proof}.json` | Passed; endpoint selection is runtime profile state, first-run setup is invite-first, provenance records general HTTP/HTTPS/WS/WSS connect policy, canonical owl launcher assets are synchronized into the generated Gradle project, and artifact proof checks the generated debug APK archive with no forbidden Python/sidecar matches plus the least-privilege `aurora-android-thin` and mobile QR/deep-link `aurora-mobile-mesh` capabilities. |
| Android arm64 physical-device APK | `android:build:thin:apk:arm64`, `android:verify:thin:apk`, then `adb install -r -t` | `dist/android-thin/arm64/aurora-android-thin-runtime-configurable-arm64-debug.apk` | Passed on Android 16/API 36 POCO X7 Pro (`2412DPC0AG`). SHA-256 `0259722c18fca3fb3f38781ac0d924c8a07840c1397a3972e3f0a5c9b5c74784`; APK contains only `arm64-v8a`, verifies with the Android debug v2 signature, and declares camera, record/modify audio, notification, foreground microphone service, network, vibration, and biometric permissions. Native scanner open/cancel removed its camera surface and restored all onboarding controls without `Saving…`, `cancelled`, or `[object Object]`; foreground WebView microphone capture opened and stopped an `AudioRecord`. |
| Android thin debug AAB | `pnpm --filter @aurora/tauri-ui android:build:thin:aab` then `android:verify:thin:aab` | `apps/aurora-tauri/reports/android-thin-aab-{build-provenance,artifact-proof}.json` | Passed; endpoint selection is runtime profile/onboarding state, provenance records general HTTP/HTTPS/WS/WSS connect policy, and artifact proof checks the generated debug AAB archive with no forbidden Python/sidecar matches plus the `aurora-android-thin` and `aurora-mobile-mesh` capabilities. |
...
| iOS thin generated overlay | `pnpm --filter @aurora/tauri-ui ios:prepare:thin` | `apps/aurora-tauri/reports/ios-thin-bundle-prepare.json` | Passed; generated general HTTP/HTTPS/WS/WSS connect policy, selected `aurora-ios-thin`/`aurora-mobile-mesh`, and declared no external binaries or resources. This is source/config proof, not an Xcode artifact. |
...
| Android thin packages | Endpoint-agnostic debug APK/AAB wrappers and artifact proof accept runtime HTTP/HTTPS/WS/WSS profiles, reject compiled operator endpoints, and scan for zero forbidden Python/sidecar matches. The current arm64 phone APK passes archive/ABI/signature/permission checks and is installed on the connected physical device. |
...
| Android preflight | **18 checks passed**; the one non-required blocked item is absent release-signing configuration in local non-strict mode. |
| iOS thin source/policy | Tauri/SDK tests, `build:frontend:ios-thin`, `ios:prepare:thin`, and `ios:policy` pass for shared iOS WebView routing, Keychain/profile/proof source invariants, runtime-configurable overlay generation, and the macOS simulator CI wrapper/provenance command. This is not Xcode build or runtime proof. |
...
- **Android thin:** WebView microphone permission decisions require trusted HTTPS/`tauri.localhost` origin, Android `RECORD_AUDIO`, foreground, and focus. The package also declares the normal `MODIFY_AUDIO_SETTINGS` permission required for WebView/WebRTC audio routing. One physical API 36 device completed a foreground capture/stop cycle. Lifecycle status still sets `backgroundWakeword=false` and `mustReleaseMicrophone=true` when backgrounded/unfocused.
...
Android native evidence covers policy/status/proof commands, build artifacts, and one physical-device foreground microphone and QR-scanner lifecycle. It does not prove background service behavior, local model execution, a durable wakeword/audio pipeline, additional OEMs, or physical-device WebRTC direct/STUN/TURN.
...
- Direct, configured-STUN, and forced-TURN foreground interop is live-proven in Chromium, Firefox, and Playwright WebKit. Android packaged-WebView UI/native behavior, one physical-device QR lifecycle, and one physical-device foreground audio capture are locally proven; its Python-peer WebRTC CI gate is implemented but not yet run. The iOS packaged-WKWebView simulator gate is also implemented but lacks a passing macOS report; desktop packaged-WebView network behavior, background/page-suspension behavior, broader physical/OEM paths, and production-scale browser certification remain unproven.
