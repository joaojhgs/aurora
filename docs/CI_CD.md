...
| --- | --- | --- |
| `quality.yml` | Fast static feedback. | Generated config check, Ruff lint/format, TypeScript typechecks. |
...
| `tauri-android.yml` | Android build and emulator smoke. | Android init, preflight report, x86_64 debug APK, emulator native payload smoke. |
...
Default desktop/mobile CI builds are unsigned and intended for validation only. Package signing, notarization, App Store Connect, and Play upload remain explicit release operations requiring platform secrets.
