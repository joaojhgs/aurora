...
| `tauri-desktop.yml` | Desktop Tauri shell and sidecar packaging smoke. | Builds the pinned, Aurora-patched static Sherpa/ONNX Runtime for Linux, macOS, and Windows; runs Rust checks and desktop-local sidecar/smoke lanes; and produces unsigned Python-free runtime-configurable desktop-client packages with artifact proof. Legacy desktop-thin scripts delegate to the client commands. |
