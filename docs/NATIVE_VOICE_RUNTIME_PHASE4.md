...
| Hosted web and WebView foreground capture | Use browser microphone capture and worker-hosted WASM modules; keep the UI thread nonblocking. | Chromium and Firefox worker-hosted VAD+Moonshine ASR parity pass with COOP/COEP and `SharedArrayBuffer`. Chromium KWS worker execution passes when the WASM KWS data archive is rebuilt with the selected full GigaSpeech BPE pack; the smaller mobile pack remains disqualified by the ONNX Runtime reshape abort. WebKit remains pending; TTS is withheld from activation. |
...
| WASM parity and browser nonblocking behavior | Partly proven | Chromium and Firefox worker-hosted VAD+ASR decoded the Moonshine test WAV to the same JFK phrase while page timers remained responsive. Chromium worker-hosted KWS passes with the selected full GigaSpeech BPE pack rebuilt into the WASM data archive, detecting `LOVELY CHILD` and `FOREVER`; the smaller mobile KWS archive is rejected by the ONNX Runtime reshape abort. WebKit evidence remains pending. |
