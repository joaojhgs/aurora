...
| --- | --- | --- |
| Desktop local | Use Rust host code for model lifecycle and native HTTP/SSE; use CPAL `0.17.3` for desktop audio capture/playback once the spike is verified. | Linux sherpa shared-library build completed. CPAL spike and full local audio path remain pending. |
