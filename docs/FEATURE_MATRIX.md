...
| WebView/browser WebRTC thin shell | Current bounded / cross-engine direct-STUN-TURN live-proven | Shared TypeScript/WebView runtime exists for hosted web thin, desktop Tauri thin, Android thin, and the iOS source path with `http-only`, `webrtc-only`, and `webrtc-preferred`. Browser-to-Python Gateway direct, STUN-reflexive, and forced-TURN interop passes in Chromium, Firefox, and Playwright WebKit with Python HTTP API disabled. WSS-only desktop/Android/iOS packaging requires no Gateway origin. Lane proof comes from browser `RTCPeerConnection.getStats()` selected pairs; packaged/mobile WebView runtime proof remains open. | [`UI_CLIENT_SURFACE_ROADMAP.md`](UI_CLIENT_SURFACE_ROADMAP.md), [`UI_CLIENT_SURFACE_STATUS.md`](UI_CLIENT_SURFACE_STATUS.md), [`WEBVIEW_WEBRTC_PROTOCOL_CONTRACT.md`](WEBVIEW_WEBRTC_PROTOCOL_CONTRACT.md), [`WEBRTC_LIVE_INTEROP_HARNESS.md`](WEBRTC_LIVE_INTEROP_HARNESS.md) |
...
| Tauri Android/iOS | Android implemented/build-proven; iOS source-implemented/build-live-limited | Android thin APK/AAB artifact proof passes and the native plugin exposes Keystore peer credentials, foreground WebView microphone policy, and lifecycle evidence. iOS has shared thin routing, device-only Keychain reconnect proof, nonsecret profiles, WSS-only-capable exact-origin build wrappers, and an existing-job MobileSafari simulator ↔ Python direct-path E2E gate. Fresh Android/iOS CI results plus iOS packaged-WKWebView/device/signing proof remain pending; durable mobile background wakeword is not claimed. | [`UI_CLIENT_SURFACE_ROADMAP.md`](UI_CLIENT_SURFACE_ROADMAP.md), [`UI_CLIENT_SURFACE_STATUS.md`](UI_CLIENT_SURFACE_STATUS.md), `apps/aurora-tauri/README.md` |
