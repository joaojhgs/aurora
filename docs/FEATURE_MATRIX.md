...
| Tauri desktop thin over HTTP/WebRTC | Implemented / build-proven | Python-free `build:bundle:desktop-thin`/`:thin` loads an async nonsecret profile for HTTP-only, WebRTC-only, or WebRTC-preferred; endpoint-agnostic AppImage/deb artifact proof rejects compiled operator endpoints and Python/sidecar content. | [`UI_CLIENT_SURFACE_STATUS.md`](UI_CLIENT_SURFACE_STATUS.md), [`TAURI_DESKTOP_BUILD.md`](TAURI_DESKTOP_BUILD.md), `apps/aurora-tauri/README.md` |
| Tauri Android/iOS | Android implemented/build-proven; iOS source-implemented/build-live-limited | Android thin APK/AAB artifact proof passes and the native plugin exposes Keystore peer credentials, room-secret storage, foreground WebView microphone policy, and lifecycle evidence. iOS has shared thin routing, device-only Keychain reconnect proof, device-only room secrets, nonsecret profiles, runtime-configurable build wrappers, and one existing macOS job that runs the complete external-Python direct-path E2E in both MobileSafari and a packaged Tauri WKWebView simulator app. Fresh Android/iOS CI results plus physical-device/STUN/TURN/signing proof remain pending; durable mobile background wakeword is not claimed. | [`UI_CLIENT_SURFACE_ROADMAP.md`](UI_CLIENT_SURFACE_ROADMAP.md), [`UI_CLIENT_SURFACE_STATUS.md`](UI_CLIENT_SURFACE_STATUS.md), `apps/aurora-tauri/README.md` |
