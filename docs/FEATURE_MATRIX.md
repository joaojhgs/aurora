...
| WebView/browser WebRTC client roles | Current bounded / cross-engine direct-STUN-TURN live-proven | Shared TypeScript/WebView runtime exists for hosted web, desktop Tauri client, Android client, and the iOS source path with `http-only`, `webrtc-only`, and `webrtc-preferred`. Runtime profiles select remote-console or mesh-node behavior independently of physical surface. Browser-to-Python Gateway direct, STUN-reflexive, and forced-TURN interop passes in Chromium, Firefox, and Playwright WebKit with Python HTTP API disabled. Desktop/Android/iOS client packages compile no operator endpoint and accept runtime HTTP/HTTPS/WS/WSS profiles. The existing Android and iOS jobs also own external-Python mobile pairing gates for packaged Android System WebView plus Chrome and iOS MobileSafari plus packaged Tauri WKWebView; those reports remain pending their first KVM/macOS runs. | [`UI_CLIENT_SURFACE_ROADMAP.md`](UI_CLIENT_SURFACE_ROADMAP.md), [`UI_CLIENT_SURFACE_STATUS.md`](UI_CLIENT_SURFACE_STATUS.md), [`WEBVIEW_WEBRTC_PROTOCOL_CONTRACT.md`](WEBVIEW_WEBRTC_PROTOCOL_CONTRACT.md), [`WEBRTC_LIVE_INTEROP_HARNESS.md`](WEBRTC_LIVE_INTEROP_HARNESS.md) |
...
| Tauri desktop client over HTTP/WebRTC | Implemented / build-proven | Python-free `build:bundle:desktop-client` loads an async nonsecret runtime profile for HTTP-only, WebRTC-only, or WebRTC-preferred remote-console/mesh-node operation; endpoint-agnostic AppImage/deb artifact proof rejects compiled operator endpoints and Python/sidecar content. Legacy `*:thin` commands delegate to the client command. | [`UI_CLIENT_SURFACE_STATUS.md`](UI_CLIENT_SURFACE_STATUS.md), [`TAURI_DESKTOP_BUILD.md`](TAURI_DESKTOP_BUILD.md), `apps/aurora-tauri/README.md` |
| Tauri Android/iOS clients | Android implemented/build-proven; iOS source-implemented/build-live-limited | Android client APK/AAB artifact proof passes and the native plugin exposes Keystore peer credentials, room-secret storage, foreground WebView microphone policy, lifecycle evidence, and bounded local capability adapters. iOS has shared client routing, device-only Keychain reconnect proof, device-only room secrets, nonsecret profiles, runtime-configurable build wrappers, and one existing macOS job that runs the complete external-Python direct-path E2E in both MobileSafari and a packaged Tauri WKWebView simulator app. Fresh Android/iOS CI results plus physical-device/STUN/TURN/signing proof remain pending; durable mobile background wakeword is not claimed. | [`UI_CLIENT_SURFACE_ROADMAP.md`](UI_CLIENT_SURFACE_ROADMAP.md), [`UI_CLIENT_SURFACE_STATUS.md`](UI_CLIENT_SURFACE_STATUS.md), `apps/aurora-tauri/README.md` |
