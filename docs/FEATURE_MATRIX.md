...
| Tauri desktop thin over HTTP/WebRTC | Implemented / build-proven | Python-free `build:bundle:desktop-thin`/`:thin` loads an async nonsecret profile for HTTP-only, WebRTC-only, or WebRTC-preferred; a WSS-only AppImage/deb build passed artifact proof with no Gateway origin and no Python/sidecar content. | [`UI_CLIENT_SURFACE_STATUS.md`](UI_CLIENT_SURFACE_STATUS.md), [`TAURI_DESKTOP_BUILD.md`](TAURI_DESKTOP_BUILD.md), `apps/aurora-tauri/README.md` |
| Tauri Android/iOS | Android implemented/build-proven; iOS source-implemented/build-live-limited | Android thin APK/AAB artifact proof passes and the native plugin exposes Keystore peer credentials, foreground WebView microphone policy, and lifecycle evidence. iOS has shared thin routing, device-only Keychain reconnect proof, nonsecret profiles, WSS-only-capable exact-origin build wrappers, and one existing macOS job that runs the complete external-Python direct-path E2E in both MobileSafari and a packaged Tauri WKWebView simulator app. Fresh Android/iOS CI results plus physical-device/STUN/TURN/signing proof remain pending; durable mobile background wakeword is not claimed. | [`UI_CLIENT_SURFACE_ROADMAP.md`](UI_CLIENT_SURFACE_ROADMAP.md), [`UI_CLIENT_SURFACE_STATUS.md`](UI_CLIENT_SURFACE_STATUS.md), `apps/aurora-tauri/README.md` |
