...
| Tauri Android/iOS | Android implemented/build-proven; iOS source-implemented/build-live-limited | Android thin APK/AAB artifact proof passes and the native plugin exposes Keystore peer credentials, foreground WebView microphone policy, and lifecycle evidence. iOS has shared thin routing, device-only Keychain reconnect proof, nonsecret profiles, and WSS-only-capable exact-origin build wrappers. Android device runtime and iOS macOS/Xcode/WKWebView/device/signing proof remain pending; durable mobile background wakeword is not claimed. | [`UI_CLIENT_SURFACE_ROADMAP.md`](UI_CLIENT_SURFACE_ROADMAP.md), [`UI_CLIENT_SURFACE_STATUS.md`](UI_CLIENT_SURFACE_STATUS.md), `apps/aurora-tauri/README.md` |
