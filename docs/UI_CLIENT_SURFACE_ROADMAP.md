...
As of 2026-07-26, the shared TypeScript/WebView WebRTC direction is no longer just roadmap work. The implemented runtime supports hosted web thin, desktop Tauri thin, Android thin, and the iOS thin source path with `http-only`, `webrtc-only`, and `webrtc-preferred` modes. Direct, configured-STUN, and forced-TURN live interop with the Python Gateway passes in Chromium, Firefox, and Playwright WebKit while the Python HTTP API is disabled. Lane classification comes from the browser `RTCPeerConnection.getStats()` selected candidate pair. Desktop and Android thin artifact proof passes without Python/sidecar content. Desktop, Android, and iOS thin packaging also accepts a WSS-only `webrtc-only` policy with no compiled HTTPS Gateway origin; a fresh desktop AppImage/deb was built and scanned in that mode. The iOS path now has device-only Keychain reconnect credentials/proof, nonsecret profile storage, exact-origin Python-free overlay generation, and a macOS simulator build/provenance lane, but no fresh Xcode artifact or WKWebView/device runtime result has been produced in this Linux environment. Physical Android/iOS runtime proof, durable mobile background wakeword, packaged-WebView network proof, and iOS signing proof remain unclaimed. See [`UI_CLIENT_SURFACE_STATUS.md`](UI_CLIENT_SURFACE_STATUS.md), [`WEBVIEW_WEBRTC_PROTOCOL_CONTRACT.md`](WEBVIEW_WEBRTC_PROTOCOL_CONTRACT.md), and [`WEBRTC_LIVE_INTEROP_HARNESS.md`](WEBRTC_LIVE_INTEROP_HARNESS.md).
...
**Current exit:** Android thin build/artifact and native-policy proof exist, but local emulator/physical runtime smoke is not proven. iOS thin source/permission/Keychain/profile wiring plus an exact-origin simulator build/provenance lane exist; macOS/Xcode build output and simulator/physical direct-STUN-TURN runtime smoke are still pending.
