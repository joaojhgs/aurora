...
- Keep the Android API 30/API 35 packaged UI/native-payload E2E and API 35 packaged System WebView ↔ Python peer WebRTC pairing/RPC/reconnect/revocation test in the existing Android pipeline; do not split its individual assertions into extra PR checks.
...
**Current exit:** Android thin build/artifact and native-policy proof exist; packaged UI/native payload passes on the local API 30 emulator, and the KVM-backed API 35 CI lane now owns packaged WebView ↔ Python WebRTC interop. A fresh CI run of that new gate and physical/OEM proof remain pending. iOS thin source/permission/Keychain/profile wiring plus an exact-origin simulator build/runtime lane exist; a fresh macOS/Xcode run and WKWebView/physical direct-STUN-TURN interop are still pending.
