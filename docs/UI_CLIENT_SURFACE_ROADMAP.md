...
- Keep the iOS simulator MobileSafari ↔ external-Python direct-path test in the existing macOS iOS workflow and on the same shared assertion/scanner contract. Treat it as browser-engine evidence only; packaged WKWebView and physical-device direct/STUN/TURN remain separate gates.
