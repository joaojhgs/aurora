...
- `packages/aurora-sdk/src/peer-host/`, `packages/aurora-sdk/src/local-tools/`, and `packages/aurora-sdk/src/lightweight-orchestrator/` own the bounded mesh-node host, local Tooling provider, and optional device assistant tier. The grant/permission authority is Rust-backed on native and WASM-backed on web. Python remains the full authoritative service runtime.
...
| Android | Tauri generated Android project, runtime-configurable HTTP/HTTPS/WS/WSS client wrappers, Android native plugin payloads, Android preflight reports, Android client bundle checks, x86_64 debug APK proof, universal four-ABI debug AAB proof, packaged API 30 emulator launch smoke, and Waydroid full-stack mesh acceptance. | Assistant role, fallback entrypoints, Android Keystore peer credentials/proofs, biometric/admin-unlock, one foreground service for microphone and connected-device reasons, lifecycle states only from native manifest payloads, Python-free artifact contents, endpoint-agnostic package policy, native mesh sessions that answer pings and the bounded `native.get_device_status` tool while the WebView is backgrounded, and real-server assistant turns after force-stop and server restart. | Waydroid is protocol/service evidence only. Physical-device Doze, OEM kill policy, battery draw, thermal behavior, signing/store release, and iOS remain separate gates. |
