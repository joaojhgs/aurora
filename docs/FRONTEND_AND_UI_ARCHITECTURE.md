...
| iOS | Shared iOS WebView thin routing, WSS-only-capable exact-origin overlay generation, device-only Keychain reconnect proof, nonsecret profile source checks on any platform, and Tauri iOS build/runtime plus MobileSafari ↔ Python direct interop gates on macOS/Xcode. | Foreground thin-shell source capability plus Keychain/profile/proof evidence; Siri/Shortcuts/App Intents, share/deep-link/widget/file-association evidence only after native targets exist and pass runtime smoke. | No local Xcode artifact or packaged WKWebView runtime proof exists in this Linux environment. MobileSafari is browser-engine evidence only. Aurora must not claim default iOS system-assistant ownership. |
