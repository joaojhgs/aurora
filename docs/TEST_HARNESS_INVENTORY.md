...
| `apps/aurora-tauri/tests/android/android-python-webrtc.e2e.test.ts` | `android:webrtc:interop` and the existing API 35 emulator step in `tauri-android.yml` | **Keep as a normal Vitest E2E test.** It reuses the same browser entry/assertions against the packaged Android System WebView, starts the external Python peer and signaling/relay fixtures, captures CDP network/runtime evidence, scans redacted reports, and cleans up every process/ADB mapping. It is intentionally part of the existing Android check rather than a one-assertion workflow. |
