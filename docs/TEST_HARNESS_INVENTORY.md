...
| --- | --- | --- |
| `apps/aurora-tauri/scripts/prepare-thin-bundle.mjs`, `build-desktop-thin-frontend.mjs`, `assert-thin-bundle-clean.mjs` | `desktop-thin-bundle-proof.test.ts`, `ci-native-evidence.test.ts`, package commands, and `tauri-desktop.yml` | **Keep as build/artifact tools.** They generate runtime-configurable Python-free overlays and inspect real bundles; converting the wrappers into test files would make the product build path less reusable. |
