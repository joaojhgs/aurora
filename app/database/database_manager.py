...
from .models import AmbientTranscription, Message
...
class DatabaseManager:
    ...
    # Ambient Transcription Methods

    async def store_ambient_transcription(self, transcription) -> bool:
        """Store an ambient transcription in the database"""
        try:
            async with aiosqlite.connect(self.db_path) as db:
                data = transcription.to_dict()
                await db.execute(
                    """
                    INSERT INTO ambient_transcriptions (
                        id, text, timestamp, chunk_id, duration, confidence,
                        embedding, metadata, session_id, source_info, created_at
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                    (
                        data["id"],
    ...
    async def get_ambient_transcriptions_for_date(self, target_date: date = None) -> list:
        ...
        try:
            async with aiosqlite.connect(self.db_path) as db:
                ...
                cursor = await db.execute(
                    """
                    SELECT * FROM ambient_transcriptions
                    WHERE timestamp BETWEEN ? AND ?
                    ORDER BY timestamp ASC
                """,
                    (start_datetime.isoformat(), end_datetime.isoformat()),
                ...
                for row in rows:
                    ...
                    transcription_data = dict(row)
                    transcriptions.append(AmbientTranscription.from_dict(transcription_data))

                return transcriptions
        except Exception as e:
            log_error(f"Error retrieving ambient transcriptions for date {target_date}: {e}")
            return []

    async def get_recent_ambient_transcriptions(self, limit: int = 50) -> list:
        ...
        try:
            async with aiosqlite.connect(self.db_path) as db:
                ...
                return list(reversed(transcriptions))  # Return in chronological order
        except Exception as e:
            log_error(f"Error retrieving recent ambient transcriptions: {e}")
            return []

    async def search_ambient_transcriptions_by_text(self, search_text: str, limit: int = 20) -> list:
    ...
    async def get_ambient_transcription_by_chunk_id(self, chunk_id: str):
        """Get a specific ambient transcription by chunk ID"""
        try:
            async with aiosqlite.connect(self.db_path) as db:
                db.row_factory = aiosqlite.Row
                cursor = await db.execute("SELECT * FROM ambient_transcriptions WHERE chunk_id = ?", (chunk_id,))
                row = await cursor.fetchone()

                if row:

                    return AmbientTranscription.from_dict(dict(row))

                return None
        except Exception as e:
            log_error(f"Error retrieving ambient transcription {chunk_id}: {e}")
            return None

    async def update_ambient_transcription_embedding(self, transcription_id: str, embedding: list[float]) -> bool:
        """Update the embedding vector for an ambient transcription"""
        try:
            async with aiosqlite.connect(self.db_path) as db:
                await db.execute(
                    "UPDATE ambient_transcriptions SET embedding = ? WHERE id = ?",
                    (json.dumps(embedding), transcription_id),
                )
                await db.commit()
                return True
        except Exception as e:
            log_error(f"Error updating ambient transcription embedding: {e}")
            return False

    async def search_ambient_transcriptions_by_similarity(
        self, query_embedding: list[float], limit: int = 20, similarity_threshold: float = 0.7
    ) -> list:
        """
        Search ambient transcriptions by vector similarity.
        Note: This is a basic implementation. For production use, consider
        using a dedicated vector database like Chroma, Pinecone, or Qdrant.
        """
        try:
            async with aiosqlite.connect(self.db_path) as db:
                db.row_factory = aiosqlite.Row
                cursor = await db.execute("SELECT * FROM ambient_transcriptions WHERE embedding IS NOT NULL")
                ...
                for row in rows:
                    ...
                    transcription = AmbientTranscription.from_dict(dict(row))
                    stored_embedding = transcription.get_embedding_vector()

                    if stored_embedding:
                        # Calculate cosine similarity
                        similarity = self._calculate_cosine_similarity(query_embedding, stored_embedding)
                        if similarity >= similarity_threshold:
                            similarities.append((similarity, transcription))

                # Sort by similarity (highest first) and return top results
                similarities.sort(key=lambda x: x[0], reverse=True)
                return [transcription for _, transcription in similarities[:limit]]
    ...
    async def cleanup_old_ambient_transcriptions(self, days_to_keep: int = 30) -> int:
        ...
        try:
            async with aiosqlite.connect(self.db_path) as db:
                cursor = await db.execute("DELETE FROM ambient_transcriptions WHERE timestamp < ?", (cutoff_date.isoformat(),))
                await db.commit()
                return cursor.rowcount
        except Exception as e:
