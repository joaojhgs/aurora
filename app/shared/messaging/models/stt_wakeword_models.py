"""STT Wake Word service message models."""
...
class WakeWordDetected(Event):
    ...
    wake_word: str = Field(description="The wake word that was detected")
    confidence: float = Field(description="Confidence score (0.0 to 1.0)")
    source: str = Field(description="Audio source where wake word was detected")
    stream_id: str = Field(description="ID of the audio stream")
    backend: WakeWordBackendType = Field(description="Backend that detected the wake word")


class WakeWordTimeout(Event):
    """Event emitted when wake word detection times out."""

    timeout_seconds: float = Field(description="How long we waited for a wake word")
