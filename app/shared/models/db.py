"""
Shared database entity models for Aurora.

Pure dataclasses and enums representing database entities.  These live in
``app.shared`` so any service can import them without cross-service imports.
"""
...
class MeshCredential:
    """Stores an outbound authentication token received from a remote mesh peer.

    After a successful pairing exchange, the remote peer issues a token.
    This instance stores the *plaintext* token so it can re-authenticate
    on reconnect without repeating the pairing flow.
    """
    ...
    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> MeshCredential:
        return cls(
            id=data["id"],
