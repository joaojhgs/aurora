...
def check_and_update_embedding_model(store: 'SQLiteVecStore', current_model_info: Dict[str, str]) -> bool:
    ...
    if stored_signature != current_signature:
        ...
        # Read existing data
        cursor = temp_connection.execute(f"""
...
class SQLiteVecStore(BaseStore):
    ...
    def put(
        ...
        vector_store.add_texts(
            texts=[text_content],
            metadatas=[metadata],
            ids=[f"{metadata['namespace']}_{key}"]
        )
    ...
    def get(
        ...
        try:
            ...
            results = vector_store.similarity_search_with_score(
                query=key,
                k=50  # Get more results to find exact match
            )
            
            for doc, score in results:
                if (doc.metadata.get("namespace") == namespace_str and 
    ...
    def delete(
        ...
        try:
            ...
            # The exact table structure may vary, but typically it's stored with a rowid or id column
...
tools_store = SQLiteVecStore(
    db_file="./data/tools.db", 
    table="tools",
    embeddings=embeddings
)
...
class CombinedSQLiteVecStore(BaseStore):
    ...
    def put(self, namespace: Tuple[str, ...], key: str, value: Dict[str, Any], index: Optional[List[str]] = None) -> None:
    ...
    async def aput(self, namespace: Tuple[str, ...], key: str, value: Dict[str, Any], index: Optional[List[str]] = None) -> None:
