...
class UIBridge(QObject):
    ...
    async def _load_today_messages(self):
        ...
        try:
            ...
            await self.bus.publish(
                DBTopics.GET_MESSAGES_FOR_DATE,
                GetMessagesForDate(date=None),
                event=False,  # Command/Query
                origin="internal"
            )
    ...
    async def _on_llm_response(self, env: Envelope) -> None:
        ...
        try:
            ...
            if hasattr(self.ui_window, 'signals'):
                log_info(f"📨 UI Bridge: Adding assistant message to UI via signal")
    ...
    async def _on_transcription(self, env: Envelope) -> None:
        ...
        try:
            ...
            if transcription.is_final:
                ...
                # Use the UI's built-in signals to add message (thread-safe)
                if hasattr(self.ui_window, 'signals'):
                    log_info(f"🎤 UI Bridge: Adding user message (STT) to UI via signal")
    ...
    async def _on_tts_started(self, env: Envelope) -> None:
        ...
        try:
            ...
            log_info(f"🔊 UI Bridge: TTS started - updating status to 'speaking'")
