...
from app.db.models import Message, CronJob
from app.messaging import Command, Envelope, MessageBus, Query, QueryResult, DBTopics
...
class GetMessagesForDate(Query):
    ...
    date: Optional[str] = None  # ISO format date string (YYYY-MM-DD), None = today
...
class DBService:
    ...
    def __init__(self, bus: MessageBus, db_path: Optional[str] = None):
    ...
    async def _store_message(self, env: Envelope) -> None:
        ...
        try:
            ...
            message = Message(
                role=cmd.role,
                content=cmd.content,
                metadata=cmd.metadata if cmd.metadata else {}
            )
    ...
    async def _store_cron_job(self, env: Envelope) -> None:
        ...
        try:
            ...
            import uuid
            from app.db.models import ScheduleType, JobStatus
