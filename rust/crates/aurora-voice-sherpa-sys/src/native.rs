...
use super::{path_bytes, pcm_len_i32, SileroVadConfig, SpeechSegment, VadError};
...
impl Detector {
    pub(crate) fn new(config: &SileroVadConfig) -> Result<Self, VadError> {
        let model =
            CString::new(path_bytes(config.model_path())).map_err(|_| VadError::InvalidConfig {
...
impl SegmentHandle {
    ...
    fn snapshot(&self) -> Result<SpeechSegment, VadError> {
        ...
        let samples = if len == 0 {
            ...
            unsafe { std::slice::from_raw_parts(raw.samples, len) }.to_vec()
        };
