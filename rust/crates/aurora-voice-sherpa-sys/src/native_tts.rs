...
use super::{
    ...
    TtsError, MAX_TTS_CALLBACK_CHUNK_SAMPLES,
};
...
impl OfflineTts {
    pub(crate) fn new(config: &OfflineTtsConfig) -> Result<Self, TtsError> {
        ...
        let data_dir = path_cstring(config.espeak_data_dir(), ErrorCode::ConfigDataDirEmpty)?;
        let lexicon = config
            .lexicon_path()
        ...
        let ptr = unsafe { SherpaOnnxCreateOfflineTts(&raw) };
    ...
    pub(crate) fn generate(
        ...
        let raw_config = SherpaOnnxGenerationConfig {
            ...
            ..SherpaOnnxGenerationConfig::default()
        };
