...
//
// forked from design-sync lib/source-kit.mjs — this repo has no per-file
// exclusion knob for the synth-entry file scan, and one source file
// (components/aurora/app-shell.tsx) imports next/link + next/navigation,
// pulling Next.js's router runtime (process.env.__NEXT_* references) into the
// single shared IIFE bundle. That throws "process is not defined" at bundle
// load time and breaks window.<GLOBAL> export for every component, not just
// this one. cfg.excludeSrcFiles (repo-relative paths) filters those files out
// of both the synth entry and the component-name derivation before either runs.
...
export async function resolvePackage(ctx) {
  ...
  // cfg.excludeSrcFiles: repo-relative paths dropped from the synth entry AND
  // component derivation before either runs (see the fork header above).
  const excludeAbs = new Set((cfg.excludeSrcFiles ?? []).map((p) => slash(resolve(PKG_DIR, p))));
  let srcFiles = srcRoot ? walk(srcRoot, (n) => /\.(tsx|jsx|mdx?)$/.test(n)) : [];
  if (excludeAbs.size) {
    const before = srcFiles.length;
    srcFiles = srcFiles.filter((p) => !excludeAbs.has(slash(p)));
    console.error(`  excludeSrcFiles: dropped ${before - srcFiles.length} file(s) from src scan`);
  }
