...
- Full `@aurora/voice-web` Vitest — 12 files, 143/143 passed.
...
- Full `@aurora/ui` Vitest — 67 files, 714/714 passed; the deterministic accessibility/responsive/visual cases remain included and passing.
- Hosted Next Assistant browser-voice Playwright — 2/2 passed across `chromium-hosted-assistant` and `chrome-android-emulated-hosted-assistant` at `2026-08-11T16:12:21.676Z` after `next build` and `next start`. Both use Chromium fake media through native `getUserMedia`, `AudioContext`, and `AudioWorklet`, with built `@aurora/voice-web` Worker/WASM assets and explicit demo transport. Fresh desktop and Pixel 5 Chromium-emulation screenshots passed direct alignment, clipping, wording, and overflow inspection. This hosted gate is Chromium-only automation: it is not hosted Firefox/WebKit coverage, a physical microphone, an OS permission prompt, a physical Android device, acoustic recognition, or local-browser STT.
...
- The browser asset-source policy was independently rejected once after it broke the real-browser offline-reopen flow, repaired, then approved. Fresh integrated verification passes focused 30/30, full package 143/143, typecheck/build, and browser storage 18/18 across Chromium, Firefox, and WebKit.
