...
const configPath = resolve(
  process.env.AURORA_TAURI_IOS_CLIENT_CONFIG_PATH
    ...
    ?? join(packageRoot, 'reports', 'generated', 'tauri.ios-client.conf.json'),
)
...
const config = {
  build: {
    beforeBuildCommand: 'pnpm build:frontend:ios-client',
  },
  app: {
    security: {
  ...
  bundle: {
    ...
    resources: {},
    longDescription:
      'Aurora iOS client packages the shared WebView HTTP/WebRTC app without Python service resources or external binaries. Gateway and signaling endpoints are configured at runtime during onboarding and are not compiled into the artifact.',
  },
}
...
console.log(`Prepared Python-free iOS client Tauri overlay: ${configPath}`)
