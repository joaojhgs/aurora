...
const prepareScript = join(packageRoot, 'scripts', 'prepare-ios-thin-bundle.mjs')
const tempDir = mkdtempSync(join(tmpdir(), 'aurora-ios-thin-simulator-'))
...
try {
  run(process.execPath, [prepareScript], {
    ...process.env,
    AURORA_TAURI_IOS_THIN_CONFIG_PATH: tempConfigPath,
    AURORA_TAURI_IOS_THIN_REPORT_PATH: tempPrepareReportPath,
  })
  ...
  writeAtomicJson(buildProvenancePath, {
    ...
    bundleMode: 'ios-thin',
    target: 'aarch64-sim',
    configPath: '<temp-ios-thin-config>',
    configSha256,
    config,
    prepareReport: {
      ...prepareReport,
    ...
    command: [
      'pnpm',
      ...buildArgs.map((value) =>
        value === tempConfigPath ? '<temp-ios-thin-config>' : value
      ),
    ],
  ...
  console.log(`iOS-thin simulator build provenance wrote ${buildProvenancePath}`)
} finally {
