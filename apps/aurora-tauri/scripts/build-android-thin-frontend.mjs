...
const origins = (process.env.AURORA_TAURI_ANDROID_ALLOWED_REMOTE_ORIGINS
...
if (!['http-only', 'webrtc-only', 'webrtc-preferred'].includes(connectionMode)) {
  throw new Error(`Invalid Android-thin connection mode: ${connectionMode}`)
}
...
function exactRemoteUrl(value) {
  if (/^(https?|wss?):$/.test(value) || value.includes('*')) {
    throw new Error(`Android-thin frontend remote origins must be exact origins, not broad/wildcard source ${value}`)
  }
  let url
  try {
    url = new URL(value)
  } catch {
    throw new Error(`Invalid Android-thin frontend remote origin: ${value}`)
  }
  if (!['https:', 'wss:'].includes(url.protocol)) {
    throw new Error(`Android-thin frontend remote origins only allow exact https:// or wss:// origins: ${value}`)
  }
  if (url.username || url.password) {
    throw new Error(`Android-thin frontend remote origins must not include credentials: ${value}`)
  }
  if (url.pathname !== '/') {
    throw new Error(`Android-thin frontend remote origins must not include non-root paths: ${value}`)
  }
  if (url.search) {
    throw new Error(`Android-thin frontend remote origins must not include query strings: ${value}`)
  }
  if (url.hash) {
    throw new Error(`Android-thin frontend remote origins must not include fragments: ${value}`)
  }
  return url
}
