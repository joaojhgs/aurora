...
const explicitArtifact = readOption('--artifact') ?? process.env.AURORA_ANDROID_THIN_ARTIFACT
...
const artifactRoot = explicitArtifact
  ...
  : resolve(process.env.AURORA_ANDROID_THIN_ARTIFACT_DIR ?? join(srcTauriRoot, 'gen', 'android', 'app', 'build', 'outputs'))
const reportPath = resolve(
  process.env.AURORA_TAURI_ANDROID_THIN_PROOF_REPORT_PATH
    ?? join(packageRoot, 'reports', `android-thin-${kind}-artifact-proof.json`),
)
...
if (failures.length) {
  console.error(`Android-thin ${kind.toUpperCase()} artifact proof failed. Wrote ${reportPath}`)
  for (const failure of failures) console.error(`- ${failure}`)
  process.exit(1)
}

console.log(`Android-thin ${kind.toUpperCase()} artifact proof passed. Wrote ${reportPath}`)
...
function checkPackageScripts() {
  ...
  for (const scriptName of [
    'android:prepare:thin',
    'android:build:thin:apk',
    'android:build:thin:aab',
    'android:verify:thin:apk',
    'android:verify:thin:aab'
  ]) {
    if (!scripts[scriptName]) failures.push(`package.json is missing ${scriptName}`)
  }
  for (const [name, value] of Object.entries(scripts)) {
    if (name.includes(':thin') && /prepare-sidecar|\bpython\b|\buv\b/i.test(String(value))) {
      failures.push(`${name} must remain Python-free and must not call uv/python/prepare-sidecar`)
    }
  }
  proof.androidThinScripts = Object.fromEntries(Object.entries(scripts).filter(([name]) => name.startsWith('android:') && name.includes(':thin')))
}
...
function checkAndroidThinConfig() {
  ...
  if (!Array.isArray(externalBin) || externalBin.length !== 0) failures.push('Android-thin config must have bundle.externalBin: []')
  if (Array.isArray(resources) ? resources.length !== 0 : Object.keys(resources).length !== 0) {
    failures.push('Android-thin config must have empty bundle.resources')
  }
...
function loadAndroidThinConfig() {
  ...
  if (!existsSync(buildProvenancePath)) {
    failures.push(`Android-thin config is missing and ${redacted(buildProvenancePath)} does not exist; run android:build:thin:${kind}`)
    return null
  }
  const provenanceRaw = readFileSync(buildProvenancePath, 'utf8')
  const provenance = JSON.parse(provenanceRaw)
  if (provenance.bundleMode !== 'android-thin') failures.push('Android-thin build provenance has wrong bundleMode')
  if (provenance.kind !== kind) failures.push(`Android-thin build provenance kind mismatch: expected ${kind}, got ${provenance.kind}`)
  if (provenance.sourceConfigWritten !== false) failures.push('Android-thin build provenance must come from a temp config, not a source-tree generated config')
  const config = provenance.config
  if (!config || typeof config !== 'object') {
    failures.push('Android-thin build provenance is missing the config snapshot')
    return null
  }
  const raw = `${JSON.stringify(config, null, 2)}\n`
  const actualSha256 = createHash('sha256').update(raw).digest('hex')
  if (provenance.configSha256 !== actualSha256) {
    failures.push('Android-thin build provenance configSha256 does not match the config snapshot')
  }
...
function checkArtifact() {
  if (!artifact || !existsSync(artifact)) {
    ...
    failures.push(`Android-thin ${kind.toUpperCase()} artifact is missing under ${artifactRoot}`)
    return
  }
...
function findArtifact(ext) {
  ...
  if (preferred.length > 1) {
    failures.push(`Ambiguous Android-thin ${ext.toUpperCase()} artifacts in preferred generated output: ${preferred.map(redacted).join(', ')}`)
    return null
  }
  if (candidates.length === 1) return candidates[0]
  if (candidates.length > 1) {
    failures.push(`Ambiguous Android-thin ${ext.toUpperCase()} artifacts under ${redacted(artifactRoot)}: ${candidates.map(redacted).join(', ')}`)
  }
  return null
}
...
function checkRuntimeConfigurableConnectSrc(csp) {
  ...
  if (!directive) {
    failures.push('Android-thin CSP must define connect-src')
    return
  }
