...
const origins = (process.env.AURORA_TAURI_IOS_ALLOWED_REMOTE_ORIGINS
...
if (!['http-only', 'webrtc-only', 'webrtc-preferred'].includes(connectionMode)) {
  throw new Error(`Invalid iOS-thin connection mode: ${connectionMode}`)
}
...
const env = {
  ...process.env,
  VITE_AURORA_RUNTIME_MODE: 'ios-thin',
...
if (result.status !== 0) {
  throw new Error(`iOS-thin frontend build failed with status ${result.status}`)
}

function exactRemoteUrl(value) {
  if (/^(https?|wss?):$/.test(value) || value.includes('*')) {
    throw new Error(`iOS-thin frontend remote origins must be exact origins, not broad/wildcard source ${value}`)
  }
  let url
  try {
    url = new URL(value)
  } catch {
    throw new Error(`Invalid iOS-thin frontend remote origin: ${value}`)
  }
  if (!['https:', 'wss:'].includes(url.protocol)) {
    throw new Error(`iOS-thin frontend remote origins only allow exact https:// or wss:// origins: ${value}`)
  }
  if (url.username || url.password) {
    throw new Error(`iOS-thin frontend remote origins must not include credentials: ${value}`)
  }
  if (url.pathname !== '/') {
    throw new Error(`iOS-thin frontend remote origins must not include non-root paths: ${value}`)
  }
  if (url.search) {
    throw new Error(`iOS-thin frontend remote origins must not include query strings: ${value}`)
  }
  if (url.hash) {
    throw new Error(`iOS-thin frontend remote origins must not include fragments: ${value}`)
  }
  return url
}
