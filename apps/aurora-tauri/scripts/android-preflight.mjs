...
const checks = [
  ...
  check('android-client-apk-arm64-release-command', packageJson.scripts?.['android:build:client:apk:arm64:release'] === 'node ./scripts/build-android-client-bundle.mjs --kind apk --target aarch64 --release', packageJson.scripts?.['android:build:client:apk:arm64:release'] ?? 'missing'),
  ...
  check('android-client-aab-release-command', packageJson.scripts?.['android:build:client:aab:release'] === 'node ./scripts/build-android-client-bundle.mjs --kind aab --release', packageJson.scripts?.['android:build:client:aab:release'] ?? 'missing'),
...
function androidNativeManifestIntegration() {
  ...
  const required = [
    ...
    'dev.aurora.tauri.nativeplugin.AuroraRuntimeForegroundService',
