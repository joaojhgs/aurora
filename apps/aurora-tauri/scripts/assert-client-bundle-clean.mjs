...
const clientConfigPath = resolve(
...
const reportPath = resolve(
  process.env.AURORA_TAURI_CLIENT_PROOF_REPORT_PATH
    ...
    ?? join(packageRoot, 'reports', 'desktop-client-bundle-proof.json'),
)
...
const proof = {
  ...
  clientConfigPath: redacted(clientConfigPath),
...
if (failures.length) {
  console.error(`Desktop client bundle proof failed. Wrote ${reportPath}`)
  for (const failure of failures) console.error(`- ${failure}`)
  process.exit(1)
}

console.log(`Desktop client bundle proof passed. Wrote ${reportPath}`)

function checkPackageScripts() {
  const packageJson = JSON.parse(readFileSync(packageJsonPath, 'utf8'))
  const scripts = packageJson.scripts ?? {}
  const script = scripts['build:bundle:desktop-client'] ?? ''
  if (!script) failures.push('package.json is missing build:bundle:desktop-client')
  if (/prepare-sidecar/.test(script)) failures.push('build:bundle:desktop-client must not call prepare-sidecar')
  if (!/tauri\.client\.conf\.json/.test(script)) failures.push('build:bundle:desktop-client must use src-tauri/tauri.client.conf.json')
  for (const [name, value] of Object.entries(scripts)) {
    if ((name.endsWith(':client') || name.endsWith(':thin')) && /prepare-sidecar|python/i.test(value)) {
      failures.push(`${name} must remain Python-free and must not call prepare-sidecar`)
    }
  }
  proof.desktopClientScript = script
}

function checkThinConfig() {
  if (!existsSync(clientConfigPath)) {
    failures.push('src-tauri/tauri.client.conf.json is missing; run prepare:bundle:desktop-client')
    return
  }
  const raw = readFileSync(clientConfigPath, 'utf8')
  const config = JSON.parse(raw)
  const externalBin = config.bundle?.externalBin ?? []
  const resources = config.bundle?.resources ?? {}
  if (!Array.isArray(externalBin) || externalBin.length !== 0) failures.push('desktop-client config must have bundle.externalBin: []')
  if (Array.isArray(resources) ? resources.length !== 0 : Object.keys(resources).length !== 0) {
    failures.push('desktop-client config must have empty bundle.resources')
  }
  if (/binaries\/aurora-sidecar|config_defaults\.json|app\/services\/config/.test(raw)) {
    failures.push('desktop-client config contains sidecar/config resource references')
  }
  ...
  if (!Array.isArray(capabilities) || capabilities.length !== 1 || capabilities[0] !== 'aurora-thin') {
    failures.push('desktop-client config must replace base capabilities with aurora-thin only')
  }
...
function checkThinCapabilityFile() {
  ...
  if (!existsSync(capabilityPath)) {
    failures.push('desktop-client capability file is missing: capabilities/aurora-thin.json')
    return
  }
  ...
  if (forbidden.length) failures.push(`desktop-client capability includes forbidden local/sidecar permissions: ${forbidden.join(', ')}`)
...
function checkBundleDir() {
  ...
  if (proof.archivesFound > 0 && proof.checkedArchives === 0) {
    failures.push('desktop-client bundle archives exist, but none were successfully inspected')
  }
}
...
function checkRuntimeConfigurableConnectSrc(csp) {
  ...
  if (!directive) {
    failures.push('desktop-client CSP must define connect-src')
    return
  }
  const sources = directive.split(/\s+/).slice(1)
  const expected = new Set(["'self'", 'http://ipc.localhost', 'http://127.0.0.1:*', 'http://localhost:*', 'ws://127.0.0.1:*', 'ws://localhost:*', 'https:', 'wss:'])
  for (const source of expected) {
    if (!sources.includes(source)) {
      failures.push(`desktop-client CSP connect-src must include runtime-configurable source ${source}`)
    }
  }
  for (const source of sources) {
    if (expected.has(source)) continue
    failures.push(`desktop-client CSP connect-src must not compile endpoint-specific source ${source}`)
  }
}
