...
Then call the JS transport command path for `getNativeCapabilityManifest()` / `androidAssistantRoleStatus`, `requestAndroidPermission('aurora.android.microphone')`, and `voiceForegroundServiceStatus`, or invoke the plugin commands from the Tauri mobile shell test harness and record the returned payload. Expected results must distinguish `roleAvailable`, `packageQualified`, `roleHeld`, `requestable`, `denied`, and `oemUnavailable`; include mic/notification/biometric/local-network/foreground-service/foreground-voice/file/share states; and keep fallback entrypoints present when `roleHeld=false`.
