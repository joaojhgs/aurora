...
The supported thin simulator package command is `ios:build:thin:simulator`. It generates a temporary Tauri overlay from exact operator HTTPS/WSS origins, excludes Python/sidecar resources and external binaries, and writes successful-build provenance. The checked-in `tauri.ios-thin.conf.json` remains a policy template; do not place bearer tokens or invite secrets in it.
