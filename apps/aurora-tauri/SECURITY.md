...
| `aurora_node_config_{get,set,delete}`, `aurora_node_config_v2_{get,set,delete}` | Loads, saves, migrates, and clears the shared nonsecret node-routing policy document. | `aurora-node-config` | Uses only the fixed legacy `aurora.nodeConfig.v1` and canonical `aurora.nodeConfig.v2` keys. Desktop reads/writes the platform keychain off the async runtime; Android uses private app storage and iOS uses UserDefaults. The document contains routing/exposure policy only, never credentials or tokens; generic credential storage permissions are not broadened. |
