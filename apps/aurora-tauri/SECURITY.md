...
| `aurora_node_config_get`, `aurora_node_config_set`, `aurora_node_config_delete` | Loads, saves, and clears the shared nonsecret node-routing policy document. | `aurora-node-config` | Uses the fixed `aurora.nodeConfig.v1` key. Desktop reads/writes the platform keychain off the async runtime; Android uses private app storage and iOS uses UserDefaults. The document contains routing/exposure policy only, never credentials or tokens; generic credential storage permissions are not broadened. |
