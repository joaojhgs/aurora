...
The Tauri event bridge does not expose Python bus objects or create a second privileged event bus. `aurora_subscribe` builds a bounded `/api/events/stream` URL from the SDK stream/topic/kind/correlation fields, starts a Rust task that consumes the Gateway SSE response, and emits each normalized backend event on a subscription-scoped Tauri event name. The SDK closes the native task through `aurora_unsubscribe` when a view closes, an assistant turn is cancelled, or the subscription iterator is returned.
