...
describe('Tauri CI native evidence contract', () => {
  ...
  it('defines a Python-free desktop client bundle lane with deterministic artifact proof and thin aliases', () => {
    ...
    expect(prepareClient).toContain("const connectSrc = [\"'self'\", 'http://127.0.0.1:*', 'http://localhost:*', 'ws://127.0.0.1:*', 'ws://localhost:*', 'https:', 'wss:']")
    ...
    expect(clientConfig).toContain("connect-src 'self' http://127.0.0.1:* http://localhost:* ws://127.0.0.1:* ws://localhost:* https: wss:")
