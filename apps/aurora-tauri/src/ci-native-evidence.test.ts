...
describe('Tauri CI native evidence contract', () => {
  ...
  it('cancels superseded pull request verification without preempting non-PR runs', () => {
    const pullRequestWorkflows = [
      ...
      ['.github/workflows/quality.yml', '${{ github.workflow }}'],
      ['.github/workflows/python-tests.yml', '${{ github.workflow }}'],
      ['.github/workflows/frontend-sdk.yml', '${{ github.workflow }}'],
      ['.github/workflows/sdk-backend-contract-conformance.yml', '${{ github.workflow }}'],
      ['.github/workflows/webrtc-interop.yml', '${{ github.workflow }}'],
      ['.github/workflows/tauri-desktop.yml', 'tauri-desktop'],
      ['.github/workflows/tauri-android.yml', 'tauri-android'],
      ['.github/workflows/tauri-ios.yml', 'tauri-ios'],
      ['.github/workflows/tauri-ios-release.yml', '${{ github.workflow }}'],
      ['.github/workflows/rust-voice.yml', '${{ github.workflow }}'],
      ['.github/workflows/required-check-aliases.yml', '${{ github.workflow }}'],
    ]

    for (const [path, groupPrefix] of pullRequestWorkflows) {
      const workflow = repoText(path)
      const expectedGroup =
        `group: ${groupPrefix}-` + '${{ github.event.pull_request.number || github.run_id }}'
  ...
  it('requires mobile preflight policy gates without claiming unsupported signing or platform capabilities', () => {
    ...
    const androidWebviewCdp = repoText('apps/aurora-tauri/scripts/android-webview-cdp.mjs')
    const androidWebviewInteropTest = repoText('apps/aurora-tauri/tests/android/android-python-webrtc.e2e.test.ts')
    ...
    expect(androidSmoke).toContain('connectAndroidWebviewCdp')
    expect(androidWebviewInteropTest).toContain('connectAndroidWebviewCdp')
    expect(androidWebviewInteropTest).toContain('Last CDP error')
    expect(androidWebviewCdp).toContain('/json/version')
    expect(androidWebviewCdp).toContain('Target.getTargets')
    expect(androidWebviewCdp).toContain('Target.attachToTarget')
    expect(androidWebviewCdp).toContain("mode === 'flattened'")
    expect(androidWebviewCdp).toContain('Target.sendMessageToTarget')
    expect(androidWebviewCdp).toContain('Target.receivedMessageFromTarget')
