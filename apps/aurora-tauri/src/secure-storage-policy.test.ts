...
describe('Tauri secure storage policy', () => {
  ...
  it('wires Android thin lifecycle and foreground microphone policy through native plugin command names', () => {
    ...
    expect(runtimeSource).toContain('backgroundWakeword?: false')
    expect(kotlinSource).toContain('ret.put("mustReleaseMicrophone", !foreground || !focused)')
    expect(kotlinSource).toContain('ret.put("backgroundWakeword", false)')
