...
describe('Tauri dev local sidecar bootstrap contract', () => {
  ...
  it('keeps package build profiles explicit while normal dev stays one command', () => {
    ...
    expect(readme).toContain('Desktop client: first-run onboarding asks for the local node name, runtime role')
    expect(readme).toContain('Legacy `*thin*` scripts remain compatibility aliases')
