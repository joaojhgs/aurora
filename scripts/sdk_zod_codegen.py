...
def render_zod_module(contract_schema: dict[str, Any]) -> str:
    ...
    # Deliberately required rather than defaulted. Mesh peers advertise this as
    # their service version, and silently falling back to a placeholder is the
    # exact defect that made every thin shell fail min_version policy.
    contract_version = contract_schema.get("contract_version")
    if not contract_version:
        raise ValueError("contract schema is missing contract_version")
    lines = [
        "/* eslint-disable */",
        ...
        f"export const AURORA_BACKEND_CONTRACT_VERSION = {json.dumps(contract_version)}",
        "",
