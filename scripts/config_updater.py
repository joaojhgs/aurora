...
def main():
    ...
    parser.add_argument('--provider', choices=['openai', 'huggingface_endpoint', 'huggingface_pipeline', 'llama_cpp'],
                       help='LLM provider type')
    parser.add_argument('--backend', choices=['cpu', 'cuda', 'rocm', 'metal', 'vulkan', 'sycl', 'rpc'],
                       help='Hardware backend for local models')
    parser.add_argument('--feature-level', choices=['core', 'full', 'dev'],
                       help='Feature level to configure')
    parser.add_argument('--setup-keys', action='store_true',
                       help='Show API key setup instructions')
    parser.add_argument('--tts-gpu', action='store_true',
                       help='Enable GPU acceleration for TTS')
    parser.add_argument('--stt-gpu', action='store_true',
                       help='Enable GPU acceleration for STT')
