...
ARTIFACT_DIR="${AURORA_HOSTED_THIN_ARTIFACT_DIR:-$ROOT/reports/hosted-thin-shell}"
RUNTIME_DIR="$(mktemp -d "${TMPDIR:-/tmp}/aurora-hosted-thin.XXXXXX")"
CONFIG_PATH="$RUNTIME_DIR/config.json"
DATA_DIR="$RUNTIME_DIR/data"
PYTHON_LOG="$ARTIFACT_DIR/python-service.log"
WEB_LOG="$ARTIFACT_DIR/web-ui.log"
GATEWAY_PORT="${AURORA_HOSTED_THIN_GATEWAY_PORT:-$(python - <<'PY'
import socket
...
WEB_PORT="${AURORA_HOSTED_THIN_WEB_PORT:-$(python - <<'PY'
import socket
with socket.socket() as sock:
    sock.bind(("127.0.0.1", 0))
    print(sock.getsockname()[1])
...
BROKER_URL="${AURORA_HOSTED_THIN_BROKER_URL:-ws://127.0.0.1:9001/mqtt}"
API_KEY="${AURORA_HOSTED_THIN_GATEWAY_API_KEY:-$(python - <<'PY'
import secrets
print("hosted-thin." + secrets.token_urlsafe(24))
PY
)}"
...
APP_ID="aurora-hosted-thin-$(python - <<'PY'
import secrets
print(secrets.token_hex(8))
PY
)"
ROOM="hosted-thin-room-$(python - <<'PY'
import secrets
print(secrets.token_hex(8))
PY
)"
...
gateway["mesh_network"]["node_name"] = "Hosted Thin E2E Python"
...
setsid env \
  NEXT_PUBLIC_AURORA_WEBRTC_THIN_CLIENT=1 \
...
AURORA_HOSTED_THIN_BASE_URL="http://127.0.0.1:$WEB_PORT" \
AURORA_HOSTED_THIN_GATEWAY_URL="http://127.0.0.1:$GATEWAY_PORT" \
AURORA_HOSTED_THIN_GATEWAY_API_KEY="$API_KEY" \
AURORA_HOSTED_THIN_BROKER_URL="$BROKER_URL" \
AURORA_HOSTED_THIN_EXPECTED_NODE="Hosted Thin E2E Python" \
AURORA_HOSTED_THIN_ARTIFACT_DIR="$ARTIFACT_DIR" \
  pnpm exec playwright test \
    --config tests/e2e/hosted_thin_shell/playwright.config.ts \
    --project chromium
